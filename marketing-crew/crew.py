...
class TheMarketingCrew():
    ...
    def head_of_marketing(self) -> Agent:
        return Agent(
            ...
            tools=[
                SerperDevTool(),       # searching the website
                ScrapeWebsiteTool(),   # scraping the website
                DirectoryReadTool('resources/drafts'), # helps to read the particular directory
                FileWriterTool(),    # write to file
                FileReadTool()       # read to file
            ],
            reasoning=True, # reasoning = thinking & then acting
            inject_date=True, # inject the current date
            llm=llm,
            allow_delegation=True,   # Allow_delegation means this agent can give tasks to others.
            max_rpm=3   # max request per minute
        )
    ...
    def content_creator_social_media(self) -> Agent:
        return Agent(
            ...
            max_iter=30,  # to put some control through max iteration. (to limit max Gemini API calls)
            max_rpm=3
        )
